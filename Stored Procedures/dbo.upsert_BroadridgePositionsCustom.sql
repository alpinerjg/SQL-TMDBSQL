...
PRINT 'upsert_BroadridgePositionsCustom("' + CAST(@UserName AS VARCHAR) + '" , "' + CAST(@UniqKey AS VARCHAR) + '") -------- START -------------'
...
IF EXISTS(SELECT 1 FROM [TMDBSQL].[dbo].[BroadridgePositionsCustom] WHERE UserName = @UserName AND [UniqKey] = @UniqKey AND ts_end IS NULL)
	BEGIN
		PRINT 'upsert_BroadridgePositionsCustom("' + CAST(@UserName AS VARCHAR) + '" , "' + CAST(@UniqKey AS VARCHAR) + '") a current record exists'
...
--									[CustomMarketPrice] ,
								...
								FROM [TMDBSQL].[dbo].[BroadridgePositionsCustom] 
				...
				DECLARE @ActualDataHash  NVARCHAR(64) = HASHBYTES('SHA2_256',@ActualData)
						...
						PRINT 'DBG: upsert_BroadridgePositionsCustom("' + CAST(@UserName AS VARCHAR) + '" , "' + CAST(@UniqKey AS VARCHAR) + '") @StagingDataHash = ' + @StagingDataHash 
						PRINT 'DBG: upsert_BroadridgePositionsCustom("' + CAST(@UserName AS VARCHAR) + '" , "' + CAST(@UniqKey AS VARCHAR) + '") @ActualDataHash = ' + @ActualDataHash
						PRINT @StagingData
						PRINT @ActualData
		
		IF @StagingDataHash <> @ActualDataHash
			BEGIN
				PRINT 'upsert_BroadridgePositionsCustom("' + CAST(@UniqKey AS VARCHAR) + '") - Giant OR failed'
				PRINT 'DBG: upsert_BroadridgePositionsCustom("' + CAST(@UniqKey AS VARCHAR) + '") @StagingDataHash = ' + @StagingDataHash 
				PRINT 'DBG: upsert_BroadridgePositionsCustom("' + CAST(@UniqKey AS VARCHAR) + '") @ActualDataHash = ' + @ActualDataHash
				PRINT @StagingData
				PRINT @ActualData

				UPDATE [TMDBSQL].[dbo].[BroadridgePositionsCustom] WITH (SERIALIZABLE)
					SET
						ts_end = @Timestamp
					WHERE
						UserName = @UserName AND [UniqKey] = @UniqKey AND ts_end IS NULL

					INSERT INTO [TMDBSQL].[dbo].[BroadridgePositionsCustom]
					(	
						[UserName] ,
						[UniqKey] ,
						ts_start ,
...
--						,@CustomMarketPrice 
			...
			END
		ELSE
			PRINT 'DBG: upsert_BroadridgePositionsCustom("' + CAST(@UniqKey AS VARCHAR) + '") No change'
	END
ELSE
	BEGIN
     -- Completely new record
					PRINT 'upsert_BroadridgePositionsCustom("' + CAST(@UniqKey AS VARCHAR) + '") - New record'
...
PRINT 'upsert_BroadridgePositionsCustom("' + CAST(@UniqKey AS VARCHAR) + '") -------- END ---------------'
