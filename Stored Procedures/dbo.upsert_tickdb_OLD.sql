...
-- =============================================
CREATE PROCEDURE [dbo].[upsert_tickdb_OLD] 
...
IF EXISTS(SELECT 1 FROM dbo.TICKDB WHERE bbsecurity = @bbsecurity)
	BEGIN
	    -- Even if it's there, make sure that it's more recent
		IF EXISTS(SELECT 1 FROM dbo.TICKDB WHERE bbsecurity = @bbsecurity AND tsbb < @tsbb)
			BEGIN
				UPDATE dbo.TICKDB WITH (SERIALIZABLE)
