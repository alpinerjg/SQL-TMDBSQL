...
-- =============================================
CREATE PROCEDURE [dbo].[upsert_tickdb_src] 
...
IF EXISTS(SELECT 1 FROM dbo.TICKDB_SRC WHERE bbsecurity = @bbsecurity AND src = @src)
	BEGIN
		...
		IF EXISTS(SELECT 1 FROM dbo.TICKDB_SRC WHERE bbsecurity = @bbsecurity AND src = @src AND tsbb < @tsbb)
			BEGIN
				UPDATE dbo.TICKDB_SRC WITH (SERIALIZABLE)
