...
-- =============================================
CREATE PROCEDURE [dbo].[upsert_BroadridgePositionsSecurity] 
     @Timestamp DateTime
	,@UserName varchar(32)
	,@UniqKey varchar(30) 
...
PRINT 'upsert_BroadridgePositionsSecurity("' + CAST(@UserName AS VARCHAR) + '" , "' + CAST(@UniqKey AS VARCHAR) + '") -------- START -------------'
...
IF EXISTS(SELECT 1 FROM [TMDBSQL].[dbo].[BroadridgePositionsSecurity] WHERE UserName = @UserName AND [UniqKey] = @UniqKey AND ts_end IS NULL)
	BEGIN
		PRINT 'upsert_BroadridgePositionsSecurity("' + CAST(@UserName AS VARCHAR) + '" , "' + CAST(@UniqKey AS VARCHAR) + '") a current record exists'
     ...
     --      ,@Position_PositionTypeString 
		...
		SELECT @ActualData = CONCAT_WS('|', 
								...
								FROM [TMDBSQL].[dbo].[BroadridgePositionsSecurity] 
		...
		IF @StagingDataHash <> @ActualDataHash
			BEGIN
				PRINT 'upsert_BroadridgePositionsSecurity("' + CAST(@UniqKey AS VARCHAR) + '") - Giant OR failed'
				PRINT 'DBG: upsert_BroadridgePositionsSecurity("' + CAST(@UniqKey AS VARCHAR) + '") @StagingDataHash = ' + @StagingDataHash 
				PRINT 'DBG: upsert_BroadridgePositionsSecurity("' + CAST(@UniqKey AS VARCHAR) + '") @ActualDataHash = ' + @ActualDataHash
				PRINT @StagingData
				PRINT @ActualData

				UPDATE [TMDBSQL].[dbo].[BroadridgePositionsSecurity] WITH (SERIALIZABLE)
					SET
						ts_end = @Timestamp
					WHERE
						UserName = @UserName AND [UniqKey] = @UniqKey AND ts_end IS NULL

					INSERT INTO [TMDBSQL].[dbo].[BroadridgePositionsSecurity]
					(	
						[UserName] ,
						[UniqKey] ,
						ts_start ,
			...
			END
		ELSE
			PRINT 'DBG: upsert_BroadridgePositionsSecurity("' + CAST(@UniqKey AS VARCHAR) + '") No change'
	END
ELSE
	BEGIN
     -- Completely new record
					PRINT 'upsert_BroadridgePositionsSecurity("' + CAST(@UniqKey AS VARCHAR) + '") - New record'
...
PRINT 'upsert_BroadridgePositionsSecurity("' + CAST(@UniqKey AS VARCHAR) + '") -------- END ---------------'
