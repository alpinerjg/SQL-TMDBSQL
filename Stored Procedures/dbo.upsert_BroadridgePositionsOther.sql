...
-- =============================================
CREATE PROCEDURE [dbo].[upsert_BroadridgePositionsOther] 
     @Timestamp DateTime
	,@UserName varchar(32)
	,@UniqKey varchar(30) 
...
PRINT 'upsert_BroadridgePositionsOther("' + CAST(@UserName AS VARCHAR) + '" , "' + CAST(@UniqKey AS VARCHAR) + '") -------- START -------------'
...
IF EXISTS(SELECT 1 FROM [TMDBSQL].[dbo].[BroadridgePositionsOther] WHERE UserName = @UserName AND [UniqKey] = @UniqKey AND ts_end IS NULL)
	BEGIN
		PRINT 'upsert_BroadridgePositionsOther("' + CAST(@UserName AS VARCHAR) + '" , "' + CAST(@UniqKey AS VARCHAR) + '") a current record exists'
     ...
     --      ,@Position_PositionTypeString 
		...
		SELECT @ActualData = CONCAT_WS('|', 
								...
								FROM [TMDBSQL].[dbo].[BroadridgePositionsOther] 
		...
		IF @StagingDataHash <> @ActualDataHash
			BEGIN
				PRINT 'upsert_BroadridgePositionsOther("' + CAST(@UniqKey AS VARCHAR) + '") - Giant OR failed'
				PRINT 'DBG: upsert_BroadridgePositionsOther("' + CAST(@UniqKey AS VARCHAR) + '") @StagingDataHash = ' + @StagingDataHash 
				PRINT 'DBG: upsert_BroadridgePositionsOther("' + CAST(@UniqKey AS VARCHAR) + '") @ActualDataHash = ' + @ActualDataHash
				PRINT @StagingData
				PRINT @ActualData

				UPDATE [TMDBSQL].[dbo].[BroadridgePositionsOther] WITH (SERIALIZABLE)
					SET
						ts_end = @Timestamp
					WHERE
						UserName = @UserName AND [UniqKey] = @UniqKey AND ts_end IS NULL

					INSERT INTO [TMDBSQL].[dbo].[BroadridgePositionsOther]
					(	
						[UserName] ,
						[UniqKey] ,
						ts_start ,
			...
			END
		ELSE
			PRINT 'DBG: upsert_BroadridgePositionsOther("' + CAST(@UniqKey AS VARCHAR) + '") No change'
	END
ELSE
	BEGIN
     -- Completely new record
					PRINT 'upsert_BroadridgePositionsOther("' + CAST(@UniqKey AS VARCHAR) + '") - New record'
...
PRINT 'upsert_BroadridgePositionsOther("' + CAST(@UniqKey AS VARCHAR) + '") -------- END ---------------'
