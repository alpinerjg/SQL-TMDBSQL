...
-- =============================================
CREATE PROCEDURE [dbo].[upsert_tickdb_src_qa] 
...
IF EXISTS(SELECT 1 FROM [TMDBSQL].[dbo].[TICKDB_SRC_QA] WHERE bbsecurity = @bbsecurity AND src = @src)
	BEGIN
	    -- Even if it's there, make sure that it's more recent
		IF EXISTS(SELECT 1 FROM dbo.TICKDB_SRC WHERE bbsecurity = @bbsecurity AND src = @src AND tsbb < @tsbb)
			BEGIN
				UPDATE [TMDBSQL].[dbo].[TICKDB_SRC_QA] WITH (SERIALIZABLE)
