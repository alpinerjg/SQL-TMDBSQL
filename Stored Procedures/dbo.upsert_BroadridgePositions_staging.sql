...
-- =============================================
CREATE PROCEDURE [dbo].[upsert_BroadridgePositions_staging] 
     @Timestamp DateTime
	,@UserName varchar(32)
	,@UniqKey varchar(30) 
...
PRINT 'upsert_BroadridgePositions_staging("' + CAST(@UserName AS VARCHAR) + '" , "' + CAST(@UniqKey AS VARCHAR) + '") -------- START -------------'
...
IF EXISTS(SELECT 1 FROM [TMDBSQL].[dbo].[BroadridgePositions] WHERE UserName = @UserName AND [UniqKey] = @UniqKey AND ts_end IS NULL)
	BEGIN
		PRINT 'upsert_BroadridgePositions_staging("' + CAST(@UserName AS VARCHAR) + '" , "' + CAST(@UniqKey AS VARCHAR) + '") a current record exists'
		...
		SELECT @ActualData = CONCAT_WS('|', 
									...
									--[Strategy_TradeStrategy_UDF_tmwUndsym] ,
									[Total_Position_Calculated_PositionCash])		
								FROM [TMDBSQL].[dbo].[BroadridgePositions] 
				...
				DECLARE @ActualDataHash  NVARCHAR(64) = HASHBYTES('SHA2_256',@ActualData)
						...
						PRINT 'DBG: upsert_BroadridgePositions_staging("' + CAST(@UserName AS VARCHAR) + '" , "' + CAST(@UniqKey AS VARCHAR) + '") @StagingDataHash = ' + @StagingDataHash 
						PRINT 'DBG: upsert_BroadridgePositions_staging("' + CAST(@UserName AS VARCHAR) + '" , "' + CAST(@UniqKey AS VARCHAR) + '") @ActualDataHash = ' + @ActualDataHash
						PRINT @StagingData
						PRINT @ActualData
		
		IF @StagingDataHash <> @ActualDataHash
			BEGIN
				PRINT 'upsert_BroadridgePositions_staging("' + CAST(@UniqKey AS VARCHAR) + '") - Giant OR failed'
				PRINT 'DBG: upsert_BroadridgePositions_staging("' + CAST(@UniqKey AS VARCHAR) + '") @StagingDataHash = ' + @StagingDataHash 
				PRINT 'DBG: upsert_BroadridgePositions_staging("' + CAST(@UniqKey AS VARCHAR) + '") @ActualDataHash = ' + @ActualDataHash
				PRINT @StagingData
				PRINT @ActualData

				UPDATE [TMDBSQL].[dbo].[BroadridgePositions] WITH (SERIALIZABLE)
					SET
						ts_end = @Timestamp
					WHERE
						UserName = @UserName AND [UniqKey] = @UniqKey AND ts_end IS NULL

					INSERT INTO [TMDBSQL].[dbo].[BroadridgePositions]
					(	
						[UserName] ,
						[UniqKey] ,
						ts_start ,
			...
			END
		ELSE
			PRINT 'DBG: upsert_BroadridgePositions_staging("' + CAST(@UniqKey AS VARCHAR) + '") No change'
	END
ELSE
	BEGIN
     -- Completely new record
					PRINT 'upsert_BroadridgePositions_staging("' + CAST(@UniqKey AS VARCHAR) + '") - New record'
...
PRINT 'upsert_BroadridgePositions_staging("' + CAST(@UniqKey AS VARCHAR) + '") -------- END ---------------'
