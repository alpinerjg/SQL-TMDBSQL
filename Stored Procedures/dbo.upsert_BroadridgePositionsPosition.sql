...
-- =============================================
CREATE PROCEDURE [dbo].[upsert_BroadridgePositionsPosition] 
     @Timestamp DateTime
	,@UserName varchar(32)
	,@UniqKey varchar(30) 
...
IF EXISTS(SELECT 1 FROM [TMDBSQL].[dbo].[BroadridgePositionsPosition] WHERE UserName = @UserName AND [UniqKey] = @UniqKey AND ts_end IS NULL)
	BEGIN
		PRINT 'upsert_BroadridgePositionsPosition("' + CAST(@UserName AS VARCHAR) + '" , "' + CAST(@UniqKey AS VARCHAR) + '") a current record exists'
     ...
     --      ,@Position_Calculated_MarketPrice 
		...
		SELECT @ActualData = CONCAT_WS('|', 
								...
								FROM [TMDBSQL].[dbo].[BroadridgePositionsPosition] 
		...
		IF @StagingDataHash <> @ActualDataHash
			BEGIN
				PRINT 'upsert_BroadridgePositionsPosition("' + CAST(@UniqKey AS VARCHAR) + '") - Giant OR failed'
				PRINT 'DBG: upsert_BroadridgePositionsPosition("' + CAST(@UniqKey AS VARCHAR) + '") @StagingDataHash = ' + @StagingDataHash 
				PRINT 'DBG: upsert_BroadridgePositionsPosition("' + CAST(@UniqKey AS VARCHAR) + '") @ActualDataHash = ' + @ActualDataHash
				PRINT @StagingData
				PRINT @ActualData

				UPDATE [TMDBSQL].[dbo].[BroadridgePositionsPosition] WITH (SERIALIZABLE)
					SET
						ts_end = @Timestamp
					WHERE
						UserName = @UserName AND [UniqKey] = @UniqKey AND ts_end IS NULL

					INSERT INTO [TMDBSQL].[dbo].[BroadridgePositionsPosition]
					(	
						[UserName] ,
						[UniqKey] ,
						ts_start ,
			...
			END
		ELSE
			PRINT 'DBG: upsert_BroadridgePositionsPosition("' + CAST(@UniqKey AS VARCHAR) + '") No change'
	END
ELSE
	BEGIN
     -- Completely new record
					PRINT 'upsert_BroadridgePositionsPosition("' + CAST(@UniqKey AS VARCHAR) + '") - New record'
...
PRINT 'upsert_BroadridgePositionsPosition("' + CAST(@UniqKey AS VARCHAR) + '") -------- END ---------------'
